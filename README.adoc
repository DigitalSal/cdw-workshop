...
// 2. http://3.109.161.118/auth/realms/partner-hols/protocol/saml/clients/cdp-sso[Workshop login] - Use this URL to authenticate yourself and enter CDP. +
1. https://attend.cloudera.com/clouderadataservicesworkshop[AMERICAS Partner Workshops] - A schedule of upcoming Cloudera Hands-On Workshops. +
...
Please use the login URL: http://3.109.161.118/auth/realms/partner-hols/protocol/saml/clients/cdp-sso[Workshop login]. +
...
=== 3.1 Open Hue for CDW Virtual Warehouse - `hive` +
...
=== 3.2 Run the following DDL in HUE for the CDW Virtual Warehouse - `hive` +
...
=== 3.3 Run the following DDL in HUE for the CDW Virtual Warehouse - `impala`. +

- Go to the page where now you will access HUE of an Impala virtual warehouse. Click on `HUE` for *`impala`* as shown in the screenshot below.
...
- Go back to the CDP Console and observe the Impala Virtual Warehouse `impala`. +
...
- Make sure that you are using the HUE of `hive`. Else, click on `HUE` and go to the HUE browser. +
...
From the cloudera console click on -  `impala`. +
...
- Open HUE for the CDW `Hive` Virtual Warehouse - `hive`
...
- Open HUE for the CDW `Impala` Virtual Warehouse - `impala`. +
...
- This can be used for both Iceberg tables and Hive Tables to improve performance. Go to the Cloudera console and look for `hive`. Click on the `Hue` button on the right upper corner of `hive` as shown in the screenshot below. +
